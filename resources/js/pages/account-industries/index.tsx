...
export default function AccountIndustries() {
            ...
            router.post(route('account-industries.store'), formData, {
                ...
                onError: (errors) => {
                    ...
                    if (typeof errors === 'string') {
                        ...
                        toast.error(t('Failed to create account industry') + ': ' + Object.values(errors).join(', '));
                    }
                }
            });
            ...
            router.put(route("account-industries.update", currentItem.id), formData, {
                ...
                onError: (errors) => {
                    ...
                    if (typeof errors === 'string') {
                        ...
                        toast.error(t('Failed to update account industry') + ': ' + Object.values(errors).join(', '));
                    }
                }
            });
        }
    ...
    const handleToggleStatus = (accountIndustry: any) => {
        const newStatus = accountIndustry.status === 'active' ? 'inactive' : 'active';
        (`${newStatus === 'active' ? t('Activating') : t('Deactivating')} account industry...`);

        router.put(route('account-industries.toggle-status', accountIndustry.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
            ...
            onError: (errors) => {
                ...
                if (typeof errors === 'string') {
                    ...
                    toast.error(t('Failed to update account industry status') + ': ' + Object.values(errors).join(', '));
                }
            }
        });
    };
    ...
    if (hasPermission(permissions, 'create-account-industries')) {
        pageActions.push({
    ...
    const breadcrumbs = [
        ...
        { title: t('Account Management'), href: route('account-industries.index') },
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-account-industries'
        },
        {
            label: t('View'),
            icon: 'Eye',
            ...
            requiredPermission: 'view-account-industries'
        },
        {
            label: t('Edit'),
            icon: 'Edit',
            action: 'edit',
            className: 'text-amber-500',
            requiredPermission: 'edit-account-industries'
        },
        {
            label: t('Delete'),
            icon: 'Trash2',
            ...
            requiredPermission: 'delete-account-industries'
        }
    ];
        ...
        >
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
                <CrudTable
                    columns={columns}
                    actions={actions}
                    data={accountIndustries?.data || []}
                    from={accountIndustries?.from || 1}
                    ...
                    entityPermissions={{
                        view: 'view-account-industries',
                        create: 'create-account-industries',
                        edit: 'edit-account-industries',
                        delete: 'delete-account-industries'
                    }}
                />
            ...
            <CrudFormModal
                ...
                title={
                    formMode === 'create'
                        ? t('Add Account Industry')
                        : formMode === 'edit'
                            ? t('Edit Account Industry')
                            : t('View Account Industry')
                }
                mode={formMode}
            />
