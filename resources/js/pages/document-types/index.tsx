...
import { Plus, Eye, Edit, Trash2, FileIcon } from 'lucide-react';
...
export default function DocumentTypes() {
    ...
    const handleAction = (action: string, item: any) => {
        ...
        switch (action) {
            case 'view':
                router.get(route('document-types.show', item.id));
                break;
            case 'edit':
                setFormMode('edit');
    ...
    const handleToggleStatus = (documentType: any) => {
        const newStatus = documentType.status === 'active' ? 'inactive' : 'active';
        toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} document type...`);

        router.put(route('document-types.toggle-status', documentType.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Document Type" button if user has permission
    ...
    const breadcrumbs = [
        ...
        { title: t('Document Management'), href: route('document-types.index') },
        { title: t('Types') }
    ];
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-document-types'
        },
        {
            label: t('View'),
            icon: 'Eye',
