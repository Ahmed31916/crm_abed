...
export default function LeadStatuses() {
    ...
    const { auth, leadStatuses, filters: pageFilters = {} } = usePage().props as any;
    ...
    const handleFormSubmit = (formData: any) => {
        if (formMode === 'create') {
            ...
            toast.loading(t('Updating lead status...'));
    ...
    const handleToggleStatus = (leadStatus: any) => {
        const newStatus = leadStatus.status === 'active' ? 'inactive' : 'active';
        toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} lead status...`);

        router.put(route('lead-statuses.toggle-status', leadStatus.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Lead Status" button if user has permission
    if (hasPermission(permissions, 'create-lead-statuses')) {
        pageActions.push({
            label: t('Add Lead Status'),
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-lead-statuses'
        },
        {
            label: t('View'),
            icon: 'Eye',
            ...
            requiredPermission: 'view-lead-statuses'
        },
        {
            label: t('Edit'),
            icon: 'Edit',
            action: 'edit',
            className: 'text-amber-500',
            requiredPermission: 'edit-lead-statuses'
        },
        {
            label: t('Delete'),
            icon: 'Trash2',
            ...
            requiredPermission: 'delete-lead-statuses'
        }
    ];
        ...
        >
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
                <CrudTable
                    columns={columns}
                    actions={actions}
                    data={leadStatuses?.data || []}
                    from={leadStatuses?.from || 1}
                    ...
                    entityPermissions={{
                        view: 'view-lead-statuses',
                        create: 'create-lead-statuses',
