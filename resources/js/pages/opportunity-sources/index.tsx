...
export default function OpportunitySources() {
    ...
    const { auth, opportunitySources, filters: pageFilters = {} } = usePage().props as any;
    ...
    const handleFormSubmit = (formData: any) => {
        if (formMode === 'create') {
            toast.loading(t('Creating opportunity source...'));
            ...
            toast.loading(t('Updating opportunity source...'));
    ...
    const handleDeleteConfirm = () => {
        toast.loading(t('Deleting opportunity source...'));
    ...
    const handleToggleStatus = (opportunitySource: any) => {
        const newStatus = opportunitySource.status === 'active' ? 'inactive' : 'active';
        toast.loading(t('{{action}} opportunity source...', { action: newStatus === 'active' ? t('Activating') : t('Deactivating') }));

        router.put(route('opportunity-sources.toggle-status', opportunitySource.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Opportunity Source" button if user has permission
    if (hasPermission(permissions, 'create-opportunity-sources')) {
        pageActions.push({
            label: t('Add Opportunity Source'),
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-opportunity-sources'
        },
        {
            label: t('View'),
            icon: 'Eye',
        ...
        >
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
                <CrudTable
                    columns={columns}
                    actions={actions}
                    data={opportunitySources?.data || []}
                    from={opportunitySources?.from || 1}
                ...
                {/* Pagination section */}
                <Pagination
                    from={opportunitySources?.from || 0}
                    to={opportunitySources?.to || 0}
                    total={opportunitySources?.total || 0}
                    links={opportunitySources?.links}
                    entityName={t("opportunity sources")}
            ...
            <CrudFormModal
                ...
                title={
                    formMode === 'create'
                        ? t('Add Opportunity Source')
                        : formMode === 'edit'
                            ? t('Edit Opportunity Source')
                            : t('View Opportunity Source')
                }
                mode={formMode}
            />
