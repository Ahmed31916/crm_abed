...
export default function LeadSources() {
    ...
    const { auth, leadSources, filters: pageFilters = {} } = usePage().props as any;
    ...
    const handleToggleStatus = (leadSource: any) => {
        const newStatus = leadSource.status === 'active' ? 'inactive' : 'active';
        toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} lead source...`);

        router.put(route('lead-sources.toggle-status', leadSource.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Lead Source" button if user has permission
    if (hasPermission(permissions, 'create-lead-sources')) {
        pageActions.push({
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-lead-sources'
        },
        {
            label: t('View'),
            icon: 'Eye',
            ...
            requiredPermission: 'view-lead-sources'
        },
        {
            label: t('Edit'),
            icon: 'Edit',
            action: 'edit',
            className: 'text-amber-500',
            requiredPermission: 'edit-lead-sources'
        },
        {
            label: t('Delete'),
            icon: 'Trash2',
            ...
            requiredPermission: 'delete-lead-sources'
        }
    ];
        ...
        >
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
                <CrudTable
                    ...
                    entityPermissions={{
                        view: 'view-lead-sources',
                        create: 'create-lead-sources',
                        edit: 'edit-lead-sources',
                        delete: 'delete-lead-sources'
                    }}
                />

                {/* Pagination section */}
                <Pagination
                    from={leadSources?.from || 0}
                    to={leadSources?.to || 0}
                    total={leadSources?.total || 0}
                    links={leadSources?.links}
