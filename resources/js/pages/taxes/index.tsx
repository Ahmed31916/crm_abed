...
export default function Taxes() {
    ...
    const { auth, taxes, filters: pageFilters = {} } = usePage().props as any;
    ...
    const handleToggleStatus = (tax: any) => {
        const newStatus = tax.status === 'active' ? 'inactive' : 'active';
        toast.loading(t('{{action}} tax...', { action: newStatus === 'active' ? t('Activating') : t('Deactivating') }));

        router.put(route('taxes.toggle-status', tax.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Tax" button if user has permission
    if (hasPermission(permissions, 'create-taxes')) {
        pageActions.push({
            label: t('Add Tax'),
    ...
    const breadcrumbs = [
        ...
        { title: t('Product Setup'), href: route('taxes.index') },
        { title: t('Taxes') }
    ];
    ...
    const columns = [
        ...
        },
        {
            key: 'type',
            label: t('Type'),
            render: (value: string) => value === 'percentage' ? t('Percentage') : t('Fixed')
        },
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-taxes'
        },
        {
            label: t('View'),
            icon: 'Eye',
        ...
        },
        {
            ...
            requiredPermission: 'delete-taxes'
        }
    ];
    ...
    return (
        ...
        >
            ...
            <CrudFormModal
                ...
                formConfig={{
                    fields: [
                        { name: 'name', label: t('Tax Name'), type: 'text', required: true },
                        { name: 'rate', label: t('Rate'), type: 'number', required: true, step: '0.0001', min: '0', max: '999.9999' },
                        {
                            name: 'type',
                            label: t('Type'),
                            type: 'select',
