...
export default function Categories() {
    ...
    const handleFormSubmit = (formData: any) => {
        if (formMode === 'create') {
            ...
            router.put(route('categories.update', currentItem.id), formData, {
                onSuccess: (page) => {
                    setIsFormModalOpen(false);
                    toast.dismiss();
                    if (page.props.flash.success) {
    ...
    const handleDeleteConfirm = () => {
        ...
        router.delete(route('categories.destroy', currentItem.id), {
            onSuccess: (page) => {
    ...
    const handleToggleStatus = (category: any) => {
        ...
        router.put(route('categories.toggle-status', category.id), {}, {
            onSuccess: (page) => {
                toast.dismiss();
                if (page.props.flash.success) {
                    toast.success(t(page.props.flash.success));
    ...
    // Add the "Add Category" button if user has permission
    if (hasPermission(permissions, 'create-categories')) {
        pageActions.push({
    ...
    const breadcrumbs = [
        ...
        { title: t('Product Setup'), href: route('categories.index') },
        { title: t('Categories') }
    ];
    ...
    const actions = [
        {
            ...
            requiredPermission: 'toggle-status-categories'
        },
        {
            label: t('View'),
            icon: 'Eye',
